...
#' 
#' @param sp an object returned by the matching.generate method
...
#' disc <- matching.arbitrate(case.prepared, nA=50000, nB=40000, nubs.A=1, phi.A=0, delta.A=0, gamma=0, delta.B=0, phi.B=0, nu.B=1, verbose=T)
#' sp <- matching.generate(case=disc, sample.A=cas1$sample.A, sample.B=cas1$sample.B, verbose=T)
#' plot.errors(sp, cas1$sample.A$sample, cas1$sample.B$sample, "dwellings", "households")
#'
#' @export
#'
#' @author Samuel Thiriot <samuel.thiriot@res-ear.ch> 
#'
#' @importFrom ggplot2 ggplot aes geom_bar xlab ylab ggtitle scale_fill_gradient2 geom_tile 
#' @importFrom gridExtra grid.arrange'
#'
plot.errors <- function(sp, sampleA, sampleB, nameA="A", nameB="B", colorRef="darkgray", colorSynthetic="blue") {

    # TODO ensure this object is of the right type
    if (class(sp) != "dpp_result") stop("the data to analyze sp should be the result of a matching.generate call")
    ...
    plot_all_errors <- ggplot(all_errors, aes(error, NRMSE)) + geom_bar(stat="identity", fill=colorSynthetic)
    ...
    plot_population_sizes <- ggplot(population_sizes, aes(x=type, y=count, fill=state)) + geom_bar(stat="identity", position = 'dodge2') + scale_gray_blue
    ...
    plot_degrees_A <- ggplot(degrees_A, aes(x=attributes, y=average.degree, fill=state)) + geom_bar(stat="identity", position = 'dodge2') + scale_gray_blue + ylab("average degree") + ggtitle(paste("average degree",nameA))
    ...
    plot_degrees_B <- ggplot(degrees_B, aes(x=attributes, y=average.degree, fill=state)) + geom_bar(stat="identity", position = 'dodge2') + scale_gray_blue + ylab("average degree") + ggtitle(paste("average degree",nameB))
    ...
    plot_frequencies_A <- ggplot(frequencies_A, aes(x=attributes, y=frequency, fill=state)) + geom_bar(stat="identity", position = 'dodge2') + scale_gray_blue + ylab("freq") + ggtitle(paste("frequencies",nameA))
    ...
    plot_frequencies_B <- ggplot(frequencies_B, aes(x=attributes, y=frequency, fill=state)) + geom_bar(stat="identity", position = 'dodge2') + scale_gray_blue + ylab("freq") + ggtitle(paste("frequencies",nameB))
    ...
    plot_pdi <- ggplot(data_hm_pdi, aes(variable, degree)) + geom_tile(aes(fill=value)) + heat_map_gradient + ggtitle(paste("difference degree for",nameA))

    diff_pdj <- sp$inputs$pdj$data - sp$gen$hat.pdj
    diff_pdj$degree <- factor(seq(0,nrow(diff_pdj)-1))
    data_hm_pdj <- melt(diff_pdj)
    plot_pdj <- ggplot(data_hm_pdj, aes(variable, degree)) + geom_tile(aes(fill=value)) + heat_map_gradient + ggtitle(paste("difference degree for",nameB))
    
    # plot pij
    diff_pij <- sp$inputs$pij$data - sp$gen$hat.pij
    diff_pij$attributesB <- row.names(diff_pij)
    data_hm_pij <- melt(diff_pij)
    plot_pij <- ggplot(data_hm_pij, aes(variable, attributesB)) + geom_tile(aes(fill=value)) + heat_map_gradient + ggtitle(paste("difference peering"))
